...
import { DinoswapMasterchef } from "./Contracts/DinoswapMasterchef";
import { DinoswapMasterchef__factory as DinoswapMasterchefFactory } from "./Contracts/factories/DinoswapMasterchef__factory";
...
export const DINOSWAP_MASTERCHEF = "0x1948abc5400aa1d72223882958da3bec643fb4e5";
...
function useContracts() {
  ...
  const [
    dinoswapMasterchef,
    setDinoswapMasterchef,
  ] = useState<DinoswapMasterchef | null>(null);
  const [
  ...
  const initContracts = async () => {
    if (providerOrSigner && addresses) {
      ...
      setDinoswapMasterchef(
        DinoswapMasterchefFactory.connect(DINOSWAP_MASTERCHEF, signer),
      );
      setSushiComplexRewarder(
